# 1. bpm_modprocesslist [流程模型中的流程过程属性表]
> 流程模型中的流程过程属性
...
> **XmlData 字段内容说明：**
> 1. `unit` 为申请人单位，与`unit_show`一个意思。若是出现`unit_show`，以`unit_show`为准
> 2. `project` 为所属项目，与`project_show`一个意思。若是出现`project_show`，以`project_show`为准
> 3. `package` 为项目包，与`package_show`一个意思。若是出现`package_show`，以`package_show`为准
> 4. `subproject` 为子项目，与`subproject_show`一个意思。若是出现`subproject_show`，以`subproject_show`为准
> 5. `errorType` 为故障类型或服务目录，与`errorType_show`一个意思。若是出现`errorType_show`，以`errorType_show`为准
> 6. `appsystem` 为应用系统
> 7. `Detail` 为详情描述

<br>

# 3. bpm_archivedData [流程实例归档表]
